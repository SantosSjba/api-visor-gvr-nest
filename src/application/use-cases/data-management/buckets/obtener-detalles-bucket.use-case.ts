...
export class ObtenerDetallesBucketUseCase {
    ...
    async execute(userId: number, bucketKey: string, region?: string): Promise<any> {
        if (!bucketKey) {
        ...
        return await this.autodeskApiService.obtenerDetallesBucket(token.tokenAcceso, bucketKey, region);
    }
}
