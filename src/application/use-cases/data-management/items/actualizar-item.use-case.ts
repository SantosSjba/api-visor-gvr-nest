...
export class ActualizarItemUseCase {
    ...
    async execute(userId: number, projectId: string, itemId: string, itemData: any): Promise<any> {
        if (!projectId) {
        ...
        return await this.autodeskApiService.actualizarItem(token.tokenAcceso, projectId, itemId, itemData);
    }
}
